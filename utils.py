...
def read_data(path, dep_path):
    """
    construct dataset with aspect tags and opinion tags
    :param path: path of data file
    :param dep_path: path of dep_data file
    :return:
    """
    
    dependencies = []
    with open(dep_path, encoding="utf-8") as fp:
        for line in fp:        
            _, depency_record = line.strip().split("####")
            dependencies.append(depency_record)
    ...
    with open(path, encoding="utf-8") as fp:
        for line in fp:
            ...
            record['dep_record'] = dep_record
            dataset.append(record)
            idx += 1
    print("N dependencies:", len(dependencies))
    print("N dataset:", len(dataset))
    assert len(dependencies) == len(dataset)
    return dataset
...
def build_dataset(ds_name, win=1, mode="train-test", test_ids=None):
    ...
    train_dep_path = './dataset/%s_train_dep.txt' % ds_name
    test_dep_path = './dataset/%s_test_dep.txt' % ds_name

    train_set = read_data(train_path, train_dep_path)
    if mode == 'train-test':
        test_set = read_data(test_path, test_dep_path)
...
def read_txt(path):
    """
    construct dataset with aspect tags
    :param path: path of data file
    :return:
    """
    dataset = []
...
def build_pos_depend(ds_name):
    ...
    train_set = read_txt(train_path)
    test_set = read_txt(test_path)
    
    return train_set,test_set

def save_dp_info(file_name,ds):
    print(len(ds))
    with open(file_name,'w') as f:
        for i in range(len(ds)):
            ...
            try:
                ...
                print('save error，ignore')
                break;

if __name__=='__main__':
    nlp = StanfordCoreNLP('http://localhost', port=9000,lang='en')
     
    init_file='14semeval_laptop'
    train,test=build_pos_depend(init_file)
     
    train_file_name='./dataset/'+init_file+'_train_dep.txt'
    test_file_name='./dataset/'+init_file+'_test_dep.txt'
     
    save_dp_info(train_file_name,train)
    save_dp_info(test_file_name,test)
