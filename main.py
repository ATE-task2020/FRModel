...
def run(args, flag2embedding_path, test_ids):
    ...
    model = MyModel(params=args, vocab=vocab, label2tag=tag_inv_vocab, pretrained_embeddings=embeddings)
    ...
    for i in range(1, n_epoch + 1):
        ...
        # ---------------training----------------
        loss, Y_pred_asp = model(dataset=train_set, is_train=True)
        ...
        print('\ttrain loss: %.2f, train precision: %.2f, train recall: %.2f, train f1: %.2f' % (loss, \
                                                                                                 p * 100, r * 100,
                                                                                                 f1 * 100))
        results_strings.append("train loss: %.2f, train precision: %.2f, train recall: %.2f, train f1: %.2f\n" % (loss, \
                                                                                                 p * 100, r * 100,
                                                                                                 f1 * 100))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-dim_asp", type=int, default=200, help="dimension aspect")
    ...
    parser.add_argument("-rnn_type", type=str, default="LSTM", help="type of rnn unit")
    ...
    flag2embedding_path = {
        'glove_840B': '/Users/FSHR/Research/OTE/embeddings/glove_840B_300d.txt'
    }
